...
fetch("Assets/sports.json")
    ...
    .then(data => {
        ...
        if (leftCol && data.length > 0) {
            const item = data[0];
            const imgSrc = item.images && item.images.length > 0 ? item.images[0] : 'https://via.placeholder.com/400x250';
            leftCol.innerHTML = `
                <div class="sports-card-large">
                    <a href="details.html?id=${item.id}" class="news-link">
                        ...
                        <span class="sports-meta">${timeAgo(item.pubdate)}</span>
                    </a>
                </div>
            `;
        }
        ...
        if (rightCol && data.length > 2) {
            ...
            rightCol.innerHTML = items.map(item => {
                const imgSrc = item.images && item.images.length > 0 ? item.images[0] : 'https://via.placeholder.com/100x75';
                return `
                <div class="sports-card-small">
                    <a href="details.html?id=${item.id}" class="news-link" style="display:flex; gap:15px; align-items:center;">
                        <img src="${imgSrc}" alt="${item.title}">
                        <div>
                            <h4>${item.title}</h4>
                            <span class="sports-meta">${timeAgo(item.pubdate)}</span>
...
fetch('Assets/business.json')
    ...
    .then(data => {
        ...
        if (middleCol && data.length > 0) {
            ...
            middleCol.innerHTML = items.map(item => {
                const imgSrc = item.images && item.images.length > 0 ? item.images[0] : 'https://via.placeholder.com/300x200';
                return `
               <div class="health-title">
                   <a href="details.html?id=${item.id}" class="news-link">
        ...
        if (rightCol && data.length > 1) {
            ...
            rightCol.innerHTML = items.map(item => `
                <div class="news-item" style="display:flex; gap:10px; margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:10px;">
                     <img src="${item.images[0]}" style="width:80px; height:60px; object-fit:cover; border-radius:4px;">
                     <div>
                         <h4 style="font-size:0.9rem; margin-bottom:5px;">${item.title}</h4>
                         <span style="font-size:0.75rem; color:#888;">${timeAgo(item.pubdate)}</span>
                     </div>
                </div>
            `).join('');
        }
...
fetch('Assets/latest.json')
    ...
    .then(data => {
        ...
        if (latestTab && data.length > 0) {
            latestTab.innerHTML = data.slice(0, 7).map(item => `
                <div class="news-item" style="margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:10px;">
                    <a href="details.html?id=${item.id}" class="news-link">
                        <h4 style="font-size:1rem; font-weight:600; margin-bottom:5px;">${item.title}</h4>
                        <span class="video-date" style="font-size:0.8rem; color:#888;">${timeAgo(item.pubdate)}</span>
                    </a>
                </div>
            `).join('');
        }
...
fetch('Assets/national.json')
    ...
    .then(data => {
        ...
        if (marketTab && data.length > 0) {
            marketTab.innerHTML = data.slice(0, 7).map(item => `
                <div class="news-item" style="margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:10px;">
                     <a href="details.html?id=${item.id}" class="news-link">
                        <h4 style="font-size:1rem; font-weight:600; margin-bottom:5px;">${item.title}</h4>
                         <span class="video-date" style="font-size:0.8rem; color:#888;">${timeAgo(item.pubdate)}</span>
...
fetch('Assets/national.json')
    ...
    .then(data => {
        ...
        if (nationalContainer && data.length > 0) {
            ...
            nationalContainer.innerHTML = items.map(item => {
                const imgSrc = item.images && item.images.length > 0 ? item.images[0] : 'https://via.placeholder.com/300x200';
                return `
...
fetch('Assets/international.json')
    .then(response => response.json())
    .then(data => {
        const internationalContainer = document.querySelector('.international-container');
        if (internationalContainer && data.length > 0) {
            const items = data.slice(0, 5);
            internationalContainer.innerHTML = items.map(item => {
                const imgSrc = item.images && item.images.length > 0 ? item.images[0] : 'https://via.placeholder.com/300x200';
                return `
    ...
    .catch(error => console.error('Error loading international section:', error));
